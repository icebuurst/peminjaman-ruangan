...
<div class="container-fluid px-4 py-3">
    ...
    <div class="mb-4" data-aos="fade-up">
        <div class="modern-tabs">
            ...
            <button class="tab-modern" onclick="filterBookings('approved', 'disetujui')">
                <i class="bi bi-check-circle me-2"></i>Disetujui
                <span class="tab-badge">{{ $bookings->whereIn('status', ['approved', 'disetujui'])->count() }}</span>
            </button>
            <button class="tab-modern" onclick="filterBookings('rejected', 'ditolak')">
                <i class="bi bi-x-circle me-2"></i>Ditolak
                <span class="tab-badge">{{ $bookings->whereIn('status', ['rejected', 'ditolak'])->count() }}</span>
            </button>
        </div>
    </div>
    @endif
...
<script>
    function filterBookings(...statuses) {
        ...
        document.querySelectorAll('.booking-item').forEach((item, index) => {
            if (statuses[0] === 'all' || statuses.includes(item.dataset.status)) {
                item.style.display = '';
